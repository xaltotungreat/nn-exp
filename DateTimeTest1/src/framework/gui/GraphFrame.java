...
public class GraphFrame extends JFrame {
	...
	public static class GraphPanel extends JPanel {
		...
		public void paintComponent(Graphics g) {
			...
			if ((minX <= 0) && (maxX >= 0)) {
				xAxisLoc = (int)((getWidth())/(maxX - minX)*Math.abs(minX));
			}
			...
			if ((minY <= 0) && (maxY >= 0)) {
				yAxisLoc = (int)((getHeight())/(maxY - minY)*Math.abs(maxY));
			}
			...
			for (Map.Entry<Double, Double> currF1 : function1.entrySet()) {
				int xC = (int)((getWidth())/(maxX - minX)*Math.abs(currF1.getKey() - minX));
				int yC = (int)((getHeight())/(maxY - minY)*Math.abs(maxY - currF1.getValue()));
			...
			for (Map.Entry<Double, Double> currF2 : function2.entrySet()) {
				int xC = (int)((getWidth())/(maxX - minX)*Math.abs(currF2.getKey() - minX));
				int yC = (int)((getHeight())/(maxY - minY)*Math.abs(maxY - currF2.getValue()));
